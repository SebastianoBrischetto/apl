...
int Fleet::getNumberOfShips(){ return fleet_.size(); }

int Fleet::getNumberOfDestroyers() {return destroyers_;}

int Fleet::getNumberOfSubmarinesAndCruisers() {return submarines_and_cruisers_;}

int Fleet::getNumberOfBattleships() {return battleships_;}

int Fleet::getNumberOfCarriers() {return carriers_;}

Ship& Fleet::getLastSunkShip(){ return last_sunk_ship_; }

bool Fleet::getIsFleetDestroyed(){ return fleet_.empty(); }

void Fleet::updateFleetNumbers(Ship& ship, int value) {
    switch(ship.getLength()){
        case DESTROYER: destroyers_ += value ; break;
        case SUBMARINE: submarines_and_cruisers_ += value; break;
        case BATTLESHIP: battleships_ += value; break;
        case CARRIER: carriers_ += value; break;
    }
}
...
Ship Fleet::createTypeOfShip(ShipType ship_type, Ocean& ocean){
    ...
    switch(ship_type){
        case DESTROYER: return Destroyer(x, y, direction, ocean);
        case SUBMARINE: return Submarine(x, y, direction, ocean);
        case CRUISER: return Cruiser(x, y, direction, ocean);
        case BATTLESHIP: return Battleship(x, y, direction, ocean);
        case CARRIER: return Carrier(x, y, direction, ocean);
        default: throw std::runtime_error("Type of ship not supported");
    }   
}
